...
namespace Maticsoft.DAL{
	 	...
	 	//tb_StockCheck
		public partial class tb_StockCheckDAL
	{
	     public string DBName = "";
         public tb_StockCheckDAL(string db)
		{
            DBName = db;     
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from [" + DBName + @"].[dbo].tb_StockCheck");
			strSql.Append(" where ");
			                                       strSql.Append(" id = @id  ");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Maticsoft.Model.tb_StockCheck model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into [" + DBName + @"].[dbo].tb_StockCheck(");			
		...
		/// 更新一条数据
		/// </summary>
		public int  Update(Maticsoft.Model.tb_StockCheck model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [" + DBName + @"].[dbo].tb_StockCheck set ");
...
SqlParameter[] parameters = {
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.tb_StockCheck GetModel(int id)
		{
			...
			strSql.Append("  from [" + DBName + @"].[dbo].tb_StockCheck ");
			strSql.Append(" where id=@id");
						SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
			...
			Maticsoft.Model.tb_StockCheck model=new Maticsoft.Model.tb_StockCheck();
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" * ");
			strSql.Append(" FROM tb_StockCheck ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
	    ...
	    public DataSet GetPageList(int pageSize, int pageIndex, string strWhere, out int iRecordCount)
        {
            ...
            return DbHelperSQL.QueryPageList("[" + DBName + @"].[dbo].tb_StockCheck", "id ","*", pageSize, pageIndex, "id desc ", strWhere, out iRecordCount);
        }
