...
{
	/// <summary>
	/// 数据访问类:tb_PurchaseOrderAccept
	/// </summary>
	public partial class tb_PurchaseOrderAcceptDAL
	{
	
         public string DBName = "";
         public tb_PurchaseOrderAcceptDAL(string db)
		{
            DBName = db;     
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from tb_PurchaseOrderAccept");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Maticsoft.Model.tb_PurchaseOrderAccept model)
		{
			StringBuilder strSql=new StringBuilder();
			...
			if (model.OrderSerialNum != null)
			{
				strSql1.Append("OrderSerialNum,");
				strSql2.Append("'"+model.OrderSerialNum+"',");
			}
			if (model.SupId != null)
			{
			...
			strSql.Append("insert into [" + DBName + @"].[dbo].tb_PurchaseOrderAccept(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Maticsoft.Model.tb_PurchaseOrderAccept model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [" + DBName + @"].[dbo].tb_PurchaseOrderAccept set ");
			if (model.SerialNum != null)
			{
			...
			if (model.OrderSerialNum != null)
			{
				strSql.Append("OrderSerialNum='"+model.OrderSerialNum+"',");
			}
			else
			{
				strSql.Append("OrderSerialNum= null ,");
			}
			if (model.SupId != null)
			{
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.tb_PurchaseOrderAccept GetModel(int id)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1  ");
			strSql.Append(" id,SerialNum,OrderSerialNum,SupId,SupCode,StoreId,StoreCode,Remark,Status,CreateUserName,CreateDate,ApprUserName,ApprDate,HandledUserName ");
			strSql.Append(" from [" + DBName + @"].[dbo].tb_PurchaseOrderAccept ");
			strSql.Append(" where id="+id+"" );
			Maticsoft.Model.tb_PurchaseOrderAccept model=new Maticsoft.Model.tb_PurchaseOrderAccept();
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.tb_PurchaseOrderAccept DataRowToModel(DataRow row)
		{
			Maticsoft.Model.tb_PurchaseOrderAccept model=new Maticsoft.Model.tb_PurchaseOrderAccept();
			if (row != null)
			{
				...
				if(row["OrderSerialNum"]!=null)
				{
					model.OrderSerialNum=row["OrderSerialNum"].ToString();
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id,SerialNum,OrderSerialNum,SupId,SupCode,StoreId,StoreCode,Remark,Status,CreateUserName,CreateDate,ApprUserName,ApprDate,HandledUserName ");
			strSql.Append(" FROM [" + DBName + @"].[dbo].tb_PurchaseOrderAccept ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM [" + DBName + @"].[dbo].tb_PurchaseOrderAccept ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from [" + DBName + @"].[dbo].tb_PurchaseOrderAccept T ");
