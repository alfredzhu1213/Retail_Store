...
namespace Common
{
    ...
    public class Utils
    {
        ...
        /// <summary>
        /// checkTags 是要检查的标签，列在其中的才会检查
        /// </summary>
        /// <param name="htmlStr"></param>
        /// <returns></returns>
        public static string AutoCheckHtml(string htmlStr)
        {

            string checkTags = "table,tbody,tr,td,p,div,span";
            Stack st = new Stack();
            Regex regStart = new Regex(@"<(?<tag>[\w]+)[\s\S]*?>");
            Regex regEnd = new Regex(@"<\/(?<tag>[\w]+)[\s\S]*?>");

            StringBuilder sb = new StringBuilder();

            string tagName;
            string[] HtmlArray = htmlStr.Split('<');
            foreach (string s in HtmlArray)
            {
                // filter first string that have no '<'
                if (sb.Length == 0)
                {
                    sb.Append(" ");
                    continue;
                ...
                Match m;
                //recode start tag
                if ((m = regStart.Match("<" + s)).Success &&
                        (tagName = m.Groups["tag"].Value) != "" &&
                        checkTags.IndexOf(tagName.ToLower()) != -1)
                {
                    st.Push(tagName);
                }
                //process end tag
                else if ((m = regEnd.Match("<" + s)).Success &&
                        (tagName = m.Groups["tag"].Value) != "" &&
                       checkTags.IndexOf(tagName.ToLower()) != -1)
                {
                    //have no start tag
                    if (st.Count == 0 || st.Peek().ToString() != tagName)
                    {
                        sb.AppendFormat("<{0}>\r\n", tagName);
                    }
                    else
            ...
            if (HtmlArray.Length == 1)
            {
                ...
                sb.Append(htmlStr); sTempHtml = sb.ToString();
            }
            else
            {
                if (HtmlArray[0].Contains("<") == false)
                {
                    sTempHtml = HtmlArray[0] + sb.ToString();
                }
                else
                {
                    sTempHtml = sb.ToString();
                }
            }
            return sTempHtml;
        }
